...
if [ ! -f "$HOME/.config/btop/btop.conf" ]; then
  # Use Onyx btop config
  mkdir -p ~/.config/btop/themes
  cp ~/.local/share/onyx/configs/btop.conf ~/.config/btop/btop.conf
fi
...
if [ -n "$THEME" ] && [ "$THEME" != ">>-skip" ]; then
  if [ -f "$ONYX_PATH/themes/$THEME/btop.theme" ]; then
    cp $ONYX_PATH/themes/$THEME/btop.theme ~/.config/btop/themes/$THEME.theme
