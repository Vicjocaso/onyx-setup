...
if [ ! -f "$HOME/.config/btop/btop.conf" ]; then
  # Use Vicjoub btop config
  mkdir -p ~/.config/btop/themes
  cp ~/.local/share/vicjoub/configs/btop.conf ~/.config/btop/btop.conf
fi
...
if [ -n "$THEME" ] && [ "$THEME" != ">>-skip" ]; then
  if [ -f "$VICJOUB_PATH/themes/$THEME/btop.theme" ]; then
    cp $VICJOUB_PATH/themes/$THEME/btop.theme ~/.config/btop/themes/$THEME.theme
